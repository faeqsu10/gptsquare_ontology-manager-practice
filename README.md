# 팔란티어 파운드리 스터디 3주차 — 온톨로지 설계와 데이터 연결

**Chapter 10 · Ontology Manager — 배치 출하 검토 실습**

2026-09-09 발표 자료입니다. 책의 설계 원칙을 바탕으로 유즈케이스를 정하고, AI FDE 활용과 직접 실습으로 객체·속성·링크·액션을 구성한 경험을 공유합니다.

## 발표 자료

- **[PowerPoint 다운로드](https://faeqsu10.github.io/gptsquare_ontology-manager-practice/ontology-manager-practice.pptx)** — 1~13장 발표, 14~27장 실습 절차, 28~35장 프롬프트. 텍스트·표 편집 가능.
- **[HTML 발표 자료 열기](https://faeqsu10.github.io/gptsquare_ontology-manager-practice/)** — 방향키로 이동, 목차에서 원하는 단계 선택, ‘발표 메모’로 설명 확인.

## 실습 참고 자료

- [처음부터 끝까지 따라하기](practice-guide.md)
- [실습 정리](practice-summary.md) · [발표 원고](presentation-notes.md)
- [AI에게 구현 맡기는 프롬프트](prompt-build.md)
- [직접 실습하도록 안내받는 프롬프트](prompt-tutor.md)
